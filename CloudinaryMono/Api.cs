...
namespace CloudinaryMono
{
	...
	public class Api
	{
		...
		string m_privateCdn;
		string m_apiAddr = "http://" + ADDR_API;
		SHA1 m_hasher;
		...
		private void WriteParam(StreamWriter writer, string key, string value)
		{
			...
			#endif
			writer.Write("--{0}{1}", HTTP_BOUNDARY, LINE_BREAK);
			writer.Write("Content-Disposition: form-data; name=\"{1}\"{0}{0}{2}{0}",
			                 LINE_BREAK,
			                 key,
			                 value);
		}
		...
		{
			writer.Write("--{0}{1}", HTTP_BOUNDARY, LINE_BREAK);
			writer.Write("Content-Disposition: form-data;  name=\"file\"; filename=\"{0}\"{1}", fileName, LINE_BREAK);
			writer.Write("Content-Type: application/octet-stream{0}", LINE_BREAK);
