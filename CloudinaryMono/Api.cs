...
namespace CloudinaryMono
{
    ...
    public class Api
    {
		...
		public HttpWebRequest GetTestRequest ()
		{
			HttpWebRequest request = HttpWebRequest.Create ("http://www.test.com") as HttpWebRequest;
			request.Method = "POST";
...
//				}
				//requestStream.Close();
				string test = "Helloe";
				requestStream.Write(GetBytes(test), 0, GetBytes(test).Length);
				requestStream.Close();
			//}
		...
		public HttpWebRequest GetRequest (HttpMethod method, string url, SortedDictionary<string, object> parameters, FileDescription file)
		{
        ...
        public HttpWebResponse Call(HttpMethod method, string url, SortedDictionary<string, object> parameters, FileDescription file)
        {
            ...
            var request = GetRequest (method, url, parameters, file);

            try
            {
//				var client = new ServiceStack.ServiceClient.Web.JsonServiceClient(url);
//				//client.set = "multipart/form-data; boundary=" + HTTP_BOUNDARY;
//				FinalizeUploadParameters (parameters);
//				var sb = new StringBuilder();
//				foreach (var param in parameters) {
//					if (!String.IsNullOrEmpty (param.Value.ToString ()))
//						WriteParam2 (sb, param.Key, param.Value.ToString ());
//				}
//
//				return client.Post<HttpWebResponse>(new CloudinaryResponse());
//
        ...
        public string BuildUploadForm(string field, string resourceType, SortedDictionary<string, object> parameters, Dictionary<string, string> htmlOptions)
        {
            ...
            builder.
                ...
                Append(ServiceStack.Text.JsonSerializer.SerializeToString(parameters)).
		...
		private void WriteParam2(StringBuilder sb, string key, string value)
		{
			#if DEBUG
			Console.WriteLine(String.Format("{0}: {1}", key, value));
			#endif
			sb.AppendLine(string.Format("--{0}", HTTP_BOUNDARY));
			sb.AppendLine(string.Format("Content-Disposition: form-data; name=\"{1}\"{0}{0}{2}",
			                 Environment.NewLine,
			                 key,
			                 value));
		}
	...
	public class CloudinaryResponse : ServiceStack.ServiceHost.IReturn<HttpWebResponse>
	{
